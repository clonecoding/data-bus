...
public class ManagerController {
    ...
    public static final String CANAL_COTEXT_DIRECTORY = SEP + "data" + SEP + "work" + SEP + "data-bus" + SEP + "cotext";


    @RequestMapping(value = "canal/{operate}", method = RequestMethod.POST)
    public String startCanal(@PathVariable String operate, @RequestBody CanalContextDto canalContextDto) {
            ...
            if (null != files && files.length > 0) {
                ...
                for (File file : files) {
                    try {
                        ...
                        canalContext.setMqProducerMsgMaxSize(Integer.valueOf(properties.getProperty("MqProducerMsgMaxSize")));
                        canalContext.setMqProducerSendMsgTimeout(Integer.valueOf(properties.getProperty("MqProducerSendMsgTimeout")));
